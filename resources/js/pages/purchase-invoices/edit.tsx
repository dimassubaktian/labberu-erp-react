...
import { edit, index, show, update } from '@/routes/purchase-invoices';
...
export default function PurchaseInvoicesEdit({
    ...
    setLayoutProps({
        breadcrumbs: [
            { title: 'Purchase Invoices', href: index() },
            {
                ...
                href: show(purchaseInvoice),
            },
            { title: 'Edit', href: edit(purchaseInvoice) },
        ],
    });
    ...
    const [taxId, setTaxId] = useState(
        purchaseInvoice.tax_id ? String(purchaseInvoice.tax_id) : 'none',
    );
    const [discountType, setDiscountType] = useState(
        purchaseInvoice.discount_type ?? 'none',
    );
    const [discountValue, setDiscountValue] = useState(
        purchaseInvoice.discount_value ?? '',
    );
            ...
            try {
                const response = (await submit(
                    purchaseOrderInvoiceItems(
                        purchaseInvoice.purchase_order.uuid,
                    ),
    ...
    return (
        <>
            <Head title={`Edit ${purchaseInvoice.purchase_invoice_code}`} />

            <div className="mx-auto w-full max-w-5xl space-y-6 p-4">
                <Heading
                    title={`Edit ${purchaseInvoice.purchase_invoice_code}`}
                    description="Update this purchase invoice"
                />

                <Form
                    noValidate
                    {...update.form(purchaseInvoice)}
                    className="space-y-6"
                >
                    {({ processing, errors }) => (
                        <>
                                ...
                                <div className="grid gap-2 sm:grid-cols-2">
                                    <div className="grid gap-2">
                                        ...
                                        <Input
                                            ...
                                            type="date"
                                            name="invoice_date"
                                            defaultValue={purchaseInvoice.invoice_date.slice(
                                                0,
                                                10,
                                            )}
                                        />
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input
                                            ...
                                            type="date"
                                            name="due_date"
                                            defaultValue={purchaseInvoice.due_date.slice(
                                                0,
                                                10,
                                            )}
                                        />
