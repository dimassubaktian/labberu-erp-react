...
export function PurchasingTab({
    ...
    const poStatusData = Object.entries(po_by_status).map(
        ([status, count]) => ({
    ...
    const grnStatusData = Object.entries(grn_by_status).map(
        ([status, count]) => ({
            name: status,
            value: count,
        ...
        <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-4">
                ...
                <KpiCard
                    ...
                    value={kpis.open_pos}
                    icon={Clock}
                    highlight={kpis.open_pos > 5 ? 'warning' : undefined}
                />
                <KpiCard
                ...
                <div className="md:col-span-2">
                    <SectionCard title={`Monthly PO Spend (${year})`}>
                        <ChartContainer
                            ...
                            config={{
                                spend: { label: 'Spend', color: '#6366f1' },
                            }}
                        >
                            <BarChart
                                data={chartData}
                    ...
                    {grnStatusData.length > 0 ? (
                        <>
                            <ChartContainer
                                ...
                                config={Object.fromEntries(
                                    Object.entries(GRN_STATUS_COLORS).map(
                                        ([k, v]) => [k, { label: k, color: v }],
                                    ),
                                )}
                            >
                                <BarChart
